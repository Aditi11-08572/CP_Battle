...
const ContestRankingTable = ({ contestName, questions = [] }) => {
  ...
  const fetchRankings = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://cp-battle.onrender.com/api/contests/${encodeURIComponent(contestName)}/rankings`, {
  ...
  useEffect(() => {
    const socket = io('https://cp-battle.onrender.com', {
